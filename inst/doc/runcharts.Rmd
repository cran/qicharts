...
Non-random variation may present itself in several ways. If the process center is shifting due to improvement or degradation we may observe _unusually_ long runs of consecutive data points on the same side of the median or that the graph crosses the median _unusually_ few times. The length of the longest run and the number of crossings in a random process are predictable within limits and depend on the total number of data points in the run chart.
...
The shift and the crossings signals are based on a false positive signal rate around 5% and have proven usefull in practice.
...
If a significant change in process perfomance has occured, it may be useful to split the graph in two.
...
By using the `data` argument, we may avoid the clumsy `$` notation. And by using the `week` column as the subgrouping vector, the `qic` function takes care of aggregating and plotting data by subgoups.
